...
model = SentenceTransformer('models/fine-tuned-search-model')


def simple_search(database, term, limit=10):
    driver = getDriver(database)
    vec = model.encode(term, convert_to_tensor=False)
    query = """
    CALL db.index.vector.queryNodes('CATEGORY_Vec', 10, $vec)
    YIELD node, score
    with node, score
    MATCH (node)<-[:HAS_VECTOR]-(c:CATEGORY)
    return c.CMID as CMID, c.CMName as CMName, node.name as matching, round(score,3) as score
    """
    result = getQuery(query, driver, params={"vec": vec}, type="df")

    return result


def embed_category_names(database, domain):

    # Connect to Neo4j
    driver = getDriver(database)

    result = getQuery(
        f"MATCH (n:{domain}) unwind n.names as name RETURN distinct n.CMID as CMID, name", driver)
    ...
    for record in result:
        ...
        if name:
            embeddings = model.encode(name, convert_to_tensor=False)
            list_embeddings = [emb.tolist() for emb in embeddings]
            vector_data.append({
                "CMID": CMID,
                "name": name,
                "name_vector": list_embeddings
            })

    query = """
            UNWIND $rows AS row
            MERGE (v:VECTOR {name: row.name})
            SET v.nameVector = row.name_vector
            WITH row, v
            MATCH (n:CATEGORY {CMID: row.CMID})
            MERGE (n)-[:HAS_VECTOR]->(v)
            return count(*) as count
            """
    result = getQuery(query, driver, params={"rows": vector_data}, type='list')

    return result

# CREATE VECTOR INDEX CATEGORY IF NOT EXISTS
# FOR (v:VECTOR)
# ON v.nameVector
# OPTIONS { indexConfig: {
