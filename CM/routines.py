...
def backup2CSV(database, mail=None):
    ...
    try:
        ...
        query_Merging = """
        with '
        MATCH (m:MERGING)-[:MERGING]->(s:STACK)-[:MERGING]->(d:DATASET)-[ru:USES]->(c:CATEGORY) OPTIONAL MATCH (d)-[cm:MERGING {stack: s.CMID, Key: ru.Key}]->(mapped:CATEGORY) OPTIONAL MATCH (s)-[rm:MERGING]->(c) RETURN m.CMID as mergingID, m.CMName as mergingName, s.CMID as stackID, s.CMName as stackName, d.CMID as datasetID, d.CMName as datasetName, head(apoc.coll.flatten(collect(rm.varName),true)) as varName, rm.transform as transform, rm.Rtransform as Rtransform, rm.Rfunction as Rfunction, rm.summaryStatistic as summaryStatistic, custom.getLabel(c) as domain, c.CMID as CMID, c.CMName as CMName, ru.Key as Key, mapped.CMID as categoryMergingCMID, mapped.CMName as categoryMergingName ORDER BY mergingName, stackName, datasetName, domain, CMName
        UNION ALL
        MATCH (m:MERGING)-[:MERGING]->(d:DATASET)-[ru:USES]->(c:CATEGORY) OPTIONAL MATCH (d)-[cm:MERGING {Key: ru.Key}]->(mapped:CATEGORY) OPTIONAL MATCH (m)-[rm:MERGING]->(c) RETURN m.CMID as mergingID, m.CMName as mergingName, "" as stackID, "" as stackName, d.CMID as datasetID, d.CMName as datasetName, head(apoc.coll.flatten(collect(rm.varName),true)) as varName, rm.transform as transform, rm.Rtransform as Rtransform, rm.Rfunction as Rfunction, rm.summaryStatistic as summaryStatistic, custom.getLabel(c) as domain, c.CMID as CMID, c.CMName as CMName, ru.Key as Key, mapped.CMID as categoryMergingCMID, mapped.CMName as categoryMergingName ORDER BY mergingName, stackName, datasetName, domain, CMName
...
def checkUSES(database, save=True, mail=None, return_type="data"):
    ...
    try:
        ...
        UNION ALL
        MATCH (c:CATEGORY)<-[r:USES]-(d:DATASET)
        WITH c, d, r, trim(toString(r.Key)) AS keyText
        WITH c, d, r, [segment IN split(keyText, " && ") | trim(segment)] AS segments
        WITH c, d, r, [segment IN segments | {segment: segment, parts: split(segment, " == ")}] AS keyParts
        WHERE any(item IN keyParts WHERE item.segment = "" OR size(item.parts) < 2 OR trim(item.parts[0]) = "" OR trim(item.parts[1]) = "")
        RETURN "Malformed Key" as error, c.CMID as CMID, c.CMName as CMName, r.Key as Key, d.CMID as datasetID, d.CMName as dataset
        UNION ALL
        MATCH (c:CATEGORY)<-[r:USES]-(d:DATASET)
        where r.Name is null or r.Name = ''
